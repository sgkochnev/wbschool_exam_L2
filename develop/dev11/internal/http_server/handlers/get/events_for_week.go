...
type GetterForWeek interface {
	EventsForWeek(userId int, date string) ([]model.Event, error)
}

// EventsForWeek handler for getting events for week
func EventsForWeek(log *slog.Logger, store GetterForWeek) http.Handler {
	eventsForWeek := func(w http.ResponseWriter, r *http.Request) {
		op := "get.EventsForDay"
		...
		event, err := store.EventsForWeek(dto.UserId, dto.Date)
		if err != nil {
	...
	return http.HandlerFunc(eventsForWeek)
}
